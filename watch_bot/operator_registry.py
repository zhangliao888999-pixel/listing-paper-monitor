...
def matches_pregrad_ramp_signature(attrs, age_minutes):
    ...
    # 的假信号,不改变原有涨幅/成交笔数门槛。
    # 2026-07-29晚间再调: v4跑了97笔后回报率持续走低,按入场流动性分桶统计发现
    # 清晰的单调关系——$0-5K均值-6.4%(49笔,占了一半样本!)、$5-10K均值-7.2%、
    # $10-20K均值+17.0%、$20K+均值+31.6%且秒崩(持仓<10秒即腰斩以上)比例是0%。
    # 原来$3000的门槛太松,大部分候选都落在表现最差的$0-5K区间。门槛提到$10000,
    # 虽然会漏掉一部分候选(相当于放弃$0-10K这个净为负的区间),但这个区间本来就是
    # 拖累整体回报率的主因,不是错杀,是止损止对了地方。
    return m5 >= 80 and n_tx >= 15 and liq >= 10000
