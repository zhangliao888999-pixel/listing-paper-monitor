...
if ($LiveMode) {
    ...
    # 2026-07-31: 用户要求并发提到6(跟云端纸盘的PREGRAD_MAX_CONCURRENT一致),
    # 单文件标记机制已改成.live_positions/目录+每币一个标记文件才撑得住>1。
    # 最大同时在场 6 x $5 = $30。
    if (-not $env:MAX_CONCURRENT_LIVE) { $env:MAX_CONCURRENT_LIVE = "6" }
    if (-not $env:LIVE_POS_SIZE_USD) { $env:LIVE_POS_SIZE_USD = "5" }
    # 2026-07-31新增: 分阶段验证——pregrad实盘先跑3笔就自动停止开新仓,人工检查
    # 没问题后把这个值调大或设0(不限)再重启。
    # 2026-07-31: 3笔限额配并发6基本测不出东西(可能一批就开满然后停机),
    # 用户拍板放到20笔——够跑满好几轮6并发,能看出滑点/胜率趋势。
    if (-not $env:LIVE_MAX_TRADES) { $env:LIVE_MAX_TRADES = "20" }
